import numpy as np
import pandas as pd
from src.weather.weather import ConcreteBuilderWeather, WeatherData
import pytest
...
@pytest.fixture
def weather_data():
    return pd.read_csv('../data/history_weather/Toronto,Ontario,CA 2021-12-15 to 2022-01-24.csv')
...
def test_concrete_weather_builder_multilabel_conditions(get_builder,
    ...
    assert len(get_weather.df.columns) == 24, 'not length of columns'
    new_columns = get_builder.convert_multilabel_encoding('conditions')
    assert len(get_weather.df.columns) > 24, 'not length of columns after encoding'
    assert len(new_columns) == 5, 'not length of columns after encoding'
    assert set(new_columns) == set(multilabel_conditions_columns), 'not equal arrays'
...
def test_concrete_weather_builder_ohe_icon(get_builder,
    ...
    assert 'clear-night' in get_weather.df.columns, 'clear-night not found in new columns'
